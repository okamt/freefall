...
import io.github.okamt.mage.VolatileDataStore
import io.github.okamt.mage.module.BuiltinType
...
import io.github.okamt.mage.util.only
...
object JamGame : InstanceDefinitionWithoutData() {
    ...
    override val defaultSpawnPoint =
        Pos(0.0, (START_Y + START_BARRIER_Y_OFFSET + 3).toDouble(), 0.0)
    ...
    enum class Zone(
        ...
        fun applyEffect(player: Player) {
            when (this) {
                ...
                GREEN -> {
                    player.sendPacket(
                        net.minestom.server.network.packet.server.play.WorldBorderLerpSizePacket(
                            RADIUS * 2.0,
                            RADIUS.toDouble() / 2.0,
                            100
                        )
    ...
    suspend fun PlayerSpawnEvent.handle() {
...
class JamPlayerData {
    companion object : VolatileDataStore<UUID, JamPlayerData>(dataMaker = ::JamPlayerData)
...
fun start() {
    ...
    registerAllBuiltinModules(BuiltinType.CORE.only)
...
val Player.op: Boolean
    get() = System.getenv("OP")?.split(",")?.let { this.username in it } == true
